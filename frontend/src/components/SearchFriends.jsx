...
export default function SearchFriends() {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/users/search-friends?query=${search}&userId=${user?.userId}`)
      const data = await res.json()
  ...
  const handleAddFriend = async (friendId) => {
    try {
      const res = await fetch('http://localhost:5000/users/add-friend', {
        method: 'POST',
